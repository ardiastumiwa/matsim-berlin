...
package org.matsim.run.carFreeMitte;
...
public final class RunCarFreeMitte {
    ...
    public static Scenario prepareScenario( Config config ) {
        ...
        var shapeFile = "original-input-data/Bezirke_-_Berlin/Berlin_Bezirke.shp";
        var features = ShapeFileReader.getAllFeatures(shapeFile);
        String gemeinde_s = "001";
        var mitte = features.stream()
                .filter(simpleFeature -> simpleFeature.getAttribute("Gemeinde_s").equals(gemeinde_s))
        ...
        for (var link : scenario.getNetwork().getLinks().entrySet()) {
            var zone = MGC.coord2Point(transformation.transform(link.getValue().getCoord()));
            if (mitte.contains(zone)) {
                link.getValue().setFreespeed(0.0001);
